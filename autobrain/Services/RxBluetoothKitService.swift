...
final class RxBluetoothKitService {
    ...
    func startScanning(withServices serviceUUIDs: [CBUUID]?) {
        ...
        .subscribeOn(MainScheduler.instance)
            .timeout(.seconds(Constant.SCANNING_TIME), scheduler: scheduler)
    ...
    // MARK: - Peripheral Connection & Discovering Services & Discovering Characteristics
    func discoverCharacteristics(for peripheral: Peripheral, withServices serviceUUIDs: [CBUUID]?, withCharacteristics characteristicUUIDs: [CBUUID]?) {
        let isConnected = peripheral.isConnected
        
        let connectedObservableCreator = { return peripheral.discoverServices(serviceUUIDs).asObservable() }
        let connectObservableCreator = {
            return peripheral.establishConnection()
                .do(onNext: { [weak self] _ in
                    self?.observeDisconnect(for: peripheral)
        ...
        let observable = isConnected ? connectedObservableCreator() : connectObservableCreator()
        let disposable = observable.flatMap { Observable.from($0) }
            .flatMap { $0.discoverCharacteristics(characteristicUUIDs) }
            .subscribe(onNext: { [unowned self] characteristics in

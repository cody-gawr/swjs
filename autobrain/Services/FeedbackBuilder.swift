...
class FeedbackBuilder: NSObject {
    public enum ReviewRequestStatus: Int {
        case neither = 0     // A user says no/no
        case negative = 1    // A user says no/yes
        case appStore = 2    // A user says yes/yes to app store
        case amazon = 3      // A user says yes/yes to amazon
        case walmart = 4     // A user says yes/yes to walmart
    }
    ...
    init(parentView: MainView, reviewType: String, client: APIClient) {
        super.init()
    ...
    private func finishReview(_ result: Int) {
        ...
        case 1:
            ...
            // TODO: (Kyle Smith) - Session.current() needs to be removed completely from this application
            Session.current().viewController?.navigationController?.pushViewController(ComposeEmailController(reviewType: reviewType!, client: api!), animated: true)
            return
        case 2:
        ...
        // switch walmart and amazon into the appropriate numbers to be stored on the backend
        let reviewRequestStatus = result > 2 ? 2 : result
        Session.current().viewController?.isShowingReview = false
        api!.updateFeedbackStatus(reviewRequestStatus: reviewRequestStatus, reviewType: reviewType!)
    }
}
